...
public interface SoftwareDeploymentTargetRepository extends JpaRepository<SoftwareDeploymentTarget, String> {
    ...
    @Query("SELECT t FROM SoftwareDeploymentTarget t WHERE t.computer.agentId = :agentId AND t.status IN :statuses AND t.deployment.status IN ('PENDING', 'IN_PROGRESS')")
